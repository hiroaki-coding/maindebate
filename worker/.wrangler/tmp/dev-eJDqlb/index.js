...
// .wrangler/tmp/bundle-vktlVX/checked-fetch.js
...
// .wrangler/tmp/bundle-vktlVX/strip-cf-connecting-ip-header.js
...
async function getFirebasePublicKeys() {
  ...
  cachedKeys = {
    keys,
    expiresAt: Date.now() + parseInt(maxAge) * 1e3
  };
  return keys;
}
...
async function authRequired(c, next) {
  ...
  try {
    ...
    const { data: user, error } = await supabase.from("users").select("*").eq("firebase_uid", decoded.uid).single();
    if (error || !user) {
    ...
    if (user.is_banned) {
      return c.json({ error: "\u3053\u306E\u30A2\u30AB\u30A6\u30F3\u30C8\u306F\u5229\u7528\u505C\u6B62\u3055\u308C\u3066\u3044\u307E\u3059" }, 403);
    }
    c.set("firebaseUid", decoded.uid);
    c.set("user", {
...
app.post("/register", authRequired, async (c) => {
  ...
  try {
    ...
    if (!c.env.SUPABASE_URL || c.env.SUPABASE_SERVICE_KEY === "PLEASE_SET_YOUR_SUPABASE_SERVICE_ROLE_KEY_HERE") {
      return c.json({
...
// .wrangler/tmp/bundle-vktlVX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vktlVX/middleware-loader.entry.ts
