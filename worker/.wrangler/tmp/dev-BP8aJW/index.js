...
// .wrangler/tmp/bundle-CQH2nb/checked-fetch.js
...
// .wrangler/tmp/bundle-CQH2nb/strip-cf-connecting-ip-header.js
...
app.post("/check-ban", async (c) => {
  const { firebaseUid } = await c.req.json();
  if (!firebaseUid) {
    return c.json({ error: "firebaseUid is required" }, 400);
  }
  const supabase = getSupabaseClient(c.env);
  const { data } = await supabase.from("users").select("is_banned").eq("firebase_uid", firebaseUid).single();
  return c.json({ isBanned: data?.is_banned ?? false });
});
app.post("/login-attempt", async (c) => {
  const { firebaseUid, success } = await c.req.json();
  if (!firebaseUid) {
    return c.json({ error: "firebaseUid is required" }, 400);
  }
...
app.delete("/me", authRequired, async (c) => {
  const user = c.get("user");
  if (!user) {
    return c.json({ error: "User not found" }, 404);
  }
  const supabase = getSupabaseClient(c.env);
  const { error } = await supabase.from("users").delete().eq("id", user.userId);
  if (error) {
...
app2.notFound((c) => c.json({ error: "Not found" }, 404));
...
// .wrangler/tmp/bundle-CQH2nb/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CQH2nb/middleware-loader.entry.ts
