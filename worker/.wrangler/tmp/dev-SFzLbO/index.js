...
// .wrangler/tmp/bundle-XpOXHO/checked-fetch.js
...
// .wrangler/tmp/bundle-XpOXHO/strip-cf-connecting-ip-header.js
...
var ALLOWED_HEADERS = ["Content-Type", "Authorization", "Accept", "Origin", "X-Requested-With"];
async function corsMiddleware(c, next) {
  ...
  const setCorsHeaders = /* @__PURE__ */ __name(() => {
...
function addCorsToResponse(c, origin) {
...
async function authRequired(c, next) {
  ...
  if (c.env.ENVIRONMENT === "development" || !c.env.SUPABASE_SERVICE_KEY || c.env.SUPABASE_SERVICE_KEY === "PLEASE_SET_YOUR_SUPABASE_SERVICE_ROLE_KEY_HERE") {
    console.log("Development mode: Using simplified auth");
    if (token === "dummy-token" || token.startsWith("ey")) {
      const mockFirebaseUid = "dev-firebase-uid-" + Math.random().toString(36).substring(2, 10);
      c.set("firebaseUid", mockFirebaseUid);
      c.set("user", null);
      await next();
      return;
    } else {
      return c.json({ error: "\u958B\u767A\u74B0\u5883: \u7121\u52B9\u306A\u30C8\u30FC\u30AF\u30F3\u3067\u3059" }, 401);
    }
  }
  try {
    return c.json({ error: "\u672C\u756A\u74B0\u5883\u306E\u8A8D\u8A3C\u306F\u672A\u5B9F\u88C5\u3067\u3059" }, 501);
  } catch (e) {
    const message = e instanceof Error ? e.message : "Invalid token";
    return c.json({ error: `\u8A8D\u8A3C\u30A8\u30E9\u30FC: ${message}` }, 401);
  }
}
__name(authRequired, "authRequired");

// src/routes/auth.ts
var app = new Hono2();
var MAX_LOGIN_ATTEMPTS = 5;
var LOCK_DURATION_MS = 60 * 1e3;
app.get("/test", (c) => {
  return c.json({
...
app.post("/register", authRequired, async (c) => {
  ...
  try {
    ...
    if (!displayName || displayName.length < 2 || displayName.length > 20) {
      return c.json({ error: "\u30CB\u30C3\u30AF\u30CD\u30FC\u30E0\u306F2\u301C20\u6587\u5B57\u3067\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044" }, 400);
    }
    ...
    if (c.env.ENVIRONMENT === "development" || !c.env.SUPABASE_SERVICE_KEY || c.env.SUPABASE_SERVICE_KEY === "PLEASE_SET_YOUR_SUPABASE_SERVICE_ROLE_KEY_HERE") {
      return c.json({
        id: "dev-user-" + firebaseUid.substring(0, 8),
        displayName,
        points: 50,
        message: "\u958B\u767A\u74B0\u5883: Supabase\u306E\u8A2D\u5B9A\u304C\u5FC5\u8981\u3067\u3059"
      }, 200);
    }
    return c.json({
      id: "temp-" + Date.now(),
...
app.get("/me", authRequired, async (c) => {
  ...
  if (!user) {
    return c.json({
      ...
      message: "Supabase\u306B\u30E6\u30FC\u30B6\u30FC\u304C\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002/api/auth/register\u3067\u767B\u9332\u3057\u3066\u304F\u3060\u3055\u3044\u3002",
      firebaseUid
    }, 404);
  }
  return c.json({
...
app.post("/login-attempt", async (c) => {
  try {
    const body = await c.req.json();
    const { firebaseUid, success } = body;
    if (!firebaseUid) {
      return c.json({ error: "firebaseUid is required" }, 400);
    }
    try {
      const key = `login:${firebaseUid}`;
      if (success) {
        await c.env.LOGIN_ATTEMPTS.delete(key);
        return c.json({ locked: false });
      }
      const existing = await c.env.LOGIN_ATTEMPTS.get(key, "json");
      const now = Date.now();
      ...
      const attempt = {
        count,
        lastAttempt: now,
        lockedUntil: count >= MAX_LOGIN_ATTEMPTS ? now + LOCK_DURATION_MS : void 0
      };
...
app.post("/lock-status", async (c) => {
  try {
    ...
    try {
      ...
      return c.json({
        locked: false,
        remainingAttempts: MAX_LOGIN_ATTEMPTS - (existing?.count ?? 0)
      });
    } catch (kvError) {
      console.error("KV operation error:", kvError);
      return c.json({ locked: false });
...
// .wrangler/tmp/bundle-XpOXHO/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XpOXHO/middleware-loader.entry.ts
