  ...
  try {
    ...
    const { error: reportError } = await supabase
      ...
      .insert({
        ...
        target_id: debateId,
        reason,
        detail: detailToStore.length > 0 ? detailToStore : null,
        status: 'pending',
      });
